...
public class DeprecateWebAppTestCase {
    ...
    private String appName = "DeprecateWebAppTestCase";
    ...
    public void testDeprecateWebApp() throws Exception {
        //Deprecate the published web app.
        HttpResponse httpResponse =
                appmPublisherRestClient.changeState(uuid,
                                                    AppmTestConstants.LifeCycleStatus.DEPRECATE);

        int httpResponseCode = httpResponse.getResponseCode();
        assertTrue(httpResponseCode == 200, httpResponseCode + " status code received.");
        JSONObject approvedResponseData = new JSONObject(httpResponse.getData());
        assertEquals(approvedResponseData.getString(AppmTestConstants.STATUS), "Success",
                     "Asset has not deprecated successfully.");
    }
