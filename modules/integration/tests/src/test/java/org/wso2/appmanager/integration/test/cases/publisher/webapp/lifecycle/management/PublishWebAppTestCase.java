...
public class PublishWebAppTestCase {

    private static final String TEST_DESCRIPTION = "Verify publishing an approved web app";
    private APPMPublisherRestClient appmPublisherRestClient;
    private String appName = "PublishWebAppTestCase";
    ...
    public void startUp() throws Exception {
        ...
        // Approve the web app.
        appmPublisherRestClient.changeState(uuid,
                                            AppmTestConstants.LifeCycleStatus.APPROVE);
    }

    @Test(description = TEST_DESCRIPTION)
    public void testPublishWebApp() throws Exception {
        // Publish the approved web app.
        HttpResponse publishedHttpResponse =
                appmPublisherRestClient.changeState(uuid, AppmTestConstants.LifeCycleStatus.PUBLISH);

        int httpResponseCode = publishedHttpResponse.getResponseCode();
        assertTrue(httpResponseCode == 200, httpResponseCode + " status code received.");
        JSONObject responseData = new JSONObject(publishedHttpResponse.getData());
        assertEquals(responseData.getString(AppmTestConstants.STATUS), "Success",
                     "Asset has not published successfully.");
    }
