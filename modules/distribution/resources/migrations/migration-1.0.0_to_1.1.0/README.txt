...
4. Go to backed up configuration folder, open app-manager.xml file and do the following changes
   ...
   - Remove below SignUp Configuration, since this has been moved into Carbon registry in App Manager 1.1.0   
...
4. Now point same WSO2 Carbon Database(User Store and Registry) and API Manager Database, Jaggery Storage and Social datasources of your APPM 1.0.0 instance to APPM 1.1.0. (Configure <APPM_1.1.0>/repository/datasource/master-datasources.xml to point same databases configured in APPM 1.0.0). 

Note : If you are using default 'WSO2CarbonDB' as the JDBCPersistenceManager datasource in <APPM_1.0.0>/repository/conf/security/application-authentication.xml, then you need to migrate the data from the Carbon datasource of APPM_1.0.0 to Carbon datasource of APPM 1.0.0.
