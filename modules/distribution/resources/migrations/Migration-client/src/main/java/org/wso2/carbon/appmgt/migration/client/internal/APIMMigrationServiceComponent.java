        ...
        try {
            if (migrateToVersion != null) {
                if (Constants.VERSION_1_2_0.equalsIgnoreCase(migrateToVersion)) {
                    log.info("Migrating WSO2 App Manager 1.1.0 to  WSO2 App Manager 1.2.0");

                    MigrationClient migrationClient = new MigrationClientImpl(tenants);

                    log.info("Migrating WSO2 App Manager 1.0.0 resources to WSO2 App Manager 1.2.0");
                  //  migrationClient.databaseMigration();
              //      migrationClient.copyNewRxtFileToRegistry();
                    migrationClient.registryResourceMigration();

                    if (log.isDebugEnabled()) {
                        log.debug("App Manager 1.1.0 to 1.1.0 migration successfully completed");
                    }
                } else {
